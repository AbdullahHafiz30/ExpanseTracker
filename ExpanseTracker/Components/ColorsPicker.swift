...
//                            let rect  = CGRect(origin: .zero, size: size)
//                            let path = Path(ellipseIn: rect)
//                            context.fill(path, with: .conicGradient(Gradient(colors: [.red,.yellow ,.green ,.cyan, .blue,.purple, .pink ]), center: CGPoint(x: size.width/2, y: size.height/2)))
//                        }
//                        .frame(width: circleSize , height: circleSize)
//
//                        Circle( )
...
//                    var alpha: CGFloat = 0
//
//                    // Convert selectedColor to UIColor for hue/saturation extraction
//                    UIColor(selectedColor).getHue(&hue, saturation: &saturation, brightness: &brightness, alpha: &alpha)
//
//                    let angle = hue * 2 * .pi
//                    let r = saturation * ((circleSize / 2) - 20)  // adjustedRadius
//                    let center = CGPoint(x: circleSize / 2, y: circleSize / 2)
//
//                    let x = center.x + cos(angle) * r
//                    let y = center.y + sin(angle) * r
