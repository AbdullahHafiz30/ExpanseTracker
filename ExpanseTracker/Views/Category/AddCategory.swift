...
struct AddCategory: View {
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                formView
                Spacer()
                CustomButton(title: "Add", action: addCategory)
            }
            .padding(.horizontal)
    ...
    private var nameAndIconView: some View {
        HStack {
            ZStack {
                Rectangle()
                    .fill(themeManager.isDarkMode && color == .black ? .white.opacity(0.3) : themeManager.isDarkMode ? color.opacity(0.3) : color.opacity(0.1))
    ...
    private var categoryIconPicker: some View {
        HStack {
            Text("Category Icon")
            Spacer()
            Image(systemName: selectedIcon)
    ...
    private var budgetLimitSlider: some View {
        VStack(alignment: .leading) {
            Text("Budget Limit")
                .padding(.horizontal)
            HStack(spacing: 20) {
